...
_.each([
	...
	test(`should parse ${JSON.stringify(testCase.input)}`, (test) => {
		test.deepEqual(parse(testCase.input), testCase.expected)
	})
})
...
_.each([
	...
	test(`should throw "${testCase.error}" given ${JSON.stringify(testCase.input)}`, (test) => {
		test.throws(() => {
