...
// @ts-expect-error ts-migrate(2451) FIXME: Cannot redeclare block-scoped variable 'childrenTr... Remove this comment to see the full error message
...
class Contract {
    ...
    getChildren(options = {}) {
        return _.reduce(this.metadata.children.map, (accumulator, contract) => {
            if (!(options as any).types || (options as any).types.has(contract.raw.type)) {
     ...
     * })
     */
    findChildrenWithCapabilities(matcher) {
        if (!matcher.raw) {
            return [];
        }
        const results = [];
            ...
            if (contract.raw.capabilities) {
                for (const capability of contract.raw.capabilities) {
                    if (matches(capability)) {
                        if (versionMatch) {
                            if (semver.valid(capability.version) && semver.validRange(versionMatch)) {
                                if (semver.satisfies(capability.version, versionMatch)) {
                                    // @ts-expect-error ts-migrate(2345) FIXME: Argument of type 'any' is not assignable to parame... Remove this comment to see the full error message
                                    results.push(contract);
                                }
                            }
                            else if (_.isEqual(capability.version, versionMatch)) {
                                // @ts-expect-error ts-migrate(2345) FIXME: Argument of type 'any' is not assignable to parame... Remove this comment to see the full error message
                                results.push(contract);
                            }
                            continue;
            ...
            if (Object.keys(matcher.raw.data).length === 1) {
                ...
                for (const childHash of hashes) {
                    ...
                    if (matches(child.raw)) {
                        if (versionMatch) {
                            if (semver.valid(child.raw.version) && semver.validRange(versionMatch)) {
    ...
    getNotSatisfiedChildRequirements(contract, options = {}) {
        ...
        // Utilities
        const shouldEvaluateType = (options as any).types
            ? (type) => {
                return (options as any).types.has(type);
            }
            : _.constant(true);
        const requirements = [];
        /**
    ...
    areChildrenSatisfied(options = {}) {
        for (const contract of this.getChildren()) {
            ...
            if ((options as any).types && utils.areSetsDisjoint((options as any).types, contract.metadata.requirements.types)) {
                continue;
            }
    ...
    static createMatcher(data, options = {}) {
        return new Contract({
            type: TYPES.MATCHER,
            operation: (options as any).operation,
            data
        });
    }
    /**
