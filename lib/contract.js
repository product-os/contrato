...
class Contract {
  /**
   * @summary Create a contract object
   * @name Contract
  ...
  findChildren (matcher) {
    ...
    for (const contract of this.getChildren().concat([ this ])) {
      ...
      const matches = _.matches(_.omit(matcher.raw.data, [ 'slug' ]))
   ...
   */
  static build (source) {
    return _.map(variants.build(source), (variant) => {
      return new Contract(variant)
    })
  }
}
